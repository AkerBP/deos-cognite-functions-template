...
class PrepareTimeSeries:
    ...
    """
    def __init__(self, client: CogniteClient, data_dict: dict):
        """Provide client and data dictionary for deployment of Cognite Function

        Args:
            client (CogniteClient): instantiated CogniteClient
            data_dict (dict): data dictionary used for Cognite Function
        """
        self.client = client
        self.input_ts_names = data_dict["ts_input_names"]
        self.output_ts = data_dict["ts_output"]
        self.data = data_dict

        self.update_ts("input_ts")
        self.update_ts("output_ts")
    ...
    def update_ts(self, field: str, val=None):
        ...
        """
        if field == "input_ts":
            self.data["input_ts"] = {str(name):{"exists":isinstance(name,str)} for name in self.input_ts_names} # include boolean to check if input is an already existing time series from CDF
        elif field == "output_ts":
            if len(self.output_ts["names"]) > len(self.output_ts["description"]) and len(self.output_ts["names"]) > len(self.output_ts["unit"]):
                # Same unit and description for all output time series
                self.output_ts["description"] = self.output_ts["description"]*len(self.output_ts["names"])
                self.output_ts["unit"] = self.output_ts["unit"]*len(self.output_ts["names"])

            self.data["output_ts"] = {name:{"exists":False,
                                            "description":desc,
                                            "unit":u} for name, desc, u in zip(self.output_ts["names"],
                                                                                self.output_ts["description"],
                                                                                self.output_ts["unit"])}
        else:
            self.data[field] = val
    ...
    def get_orig_timeseries(self, calc_func) -> dict:
        ...
        ts_inputs = self.data["input_ts"]
        ts_outputs = self.data["output_ts"]

        end_date = pd.Timestamp.now(tz="CET").floor("1s").tz_convert("UTC") #- timedelta(days=2) #TODO: SUBTRACTING ONE DAY ONLY FOR TESTING !
        start_date = end_date - timedelta(minutes=int(self.data["cron_interval_in_minutes"]))
        ...
        self.data["input_ts_data"] = {ts_name: [] for ts_name in ts_inputs.keys()} # stores original signal only for current date
        ...
        while not nonan:
            ...
            df_orig = client.time_series.data.retrieve_dataframe_in_tz(external_id=all_orig_extid,
                                                                    granularity=self.data["input_ts_sampling_rate"],
                                                                    aggregates="average",
        ...
        latest_start = latest_end - timedelta(minutes=int(self.data["cron_interval_in_minutes"]))

        self.data["input_ts"] = ts_inputs # update dictionary
        ...
        for ts_in, ts_out in zip(ts_inputs.keys(), ts_output_names):
            ...
            if not data_in["exists"]:
                self.data["input_ts_data"][ts_in] = float(ts_in)
                continue # input not in CDF, provided separately, skip to next input
            ...
            if "aggregate" in self.data["optional"]: # append data part of aggregation period prior to schedule start to data part of shcedule
                ...
                if start_time_aggregate < start_time_schedule:
                    orig_extid = self.data["input_ts"][ts_in]["orig_extid"]
            ...
            self.data["input_ts_data"][ts_in] = df_orig[ts_in]
    ...
    def create_timeseries(self) -> list:
        ...
        client = self.client
        data = self.data
        input_ts = data["input_ts"]
        output_ts = data["output_ts"]

        if len(input_ts.keys()) > len(output_ts.keys()): # Assume all input time series are from same asset
            asset_ids = [client.time_series.list(name=list(input_ts.keys())[0])[0].asset_id]
        else: # same num input and output signals --> output time series linked to asset of associated input time series
            asset_ids = [client.time_series.list(name=ts_name)[0].asset_id for ts_name in input_ts]

        for ts_out_name, asset_id in zip(output_ts.keys(), asset_ids):
            if not output_ts[ts_out_name]["exists"]:
                ...
                client.time_series.create(TimeSeries(
                                                    ...
                                                    asset_id=asset_id,
                                                    unit=output_ts[ts_out_name]["unit"],
                                                    description=output_ts[ts_out_name]["description"]))
    ...
    def retrieve_orig_ts(self, ts_in: str, ts_out: str, latest_start=None, latest_end=None) -> pd.DataFrame:
        ...
        client = self.client
        data = self.data
        data_in = self.data["input_ts"][ts_in]
        data_out = self.data["output_ts"][ts_out]
        ...
        try:
            ...
            if not data_out["exists"]:
                if "historic_start_time" in data["optional"]: # retrieve original signal from specified date
                    ...
                    start_date = client.time_series.data.retrieve_dataframe(external_id=orig_extid,
                                                                            aggregates="average",
                                                                            granularity=data["input_ts_sampling_rate"]).index[0]
            ...
            df = client.time_series.data.retrieve_dataframe_in_tz(external_id=orig_extid,
                                                                    aggregates="average",
                                                                    granularity=data["input_ts_sampling_rate"],
            ...
            while df.empty:
                ...
                start_date = end_date_new - timedelta(minutes=int(data["cron_interval_in_minutes"]))
    ...
    def align_time_series(self, ts_all: list) -> list:
        ...
        time_index = pd.date_range(start=latest_start_date, end=earliest_end_date, freq=self.data["input_ts_sampling_rate"][:-1]+AGG_PERIOD[self.data["input_ts_sampling_rate"][-1]])
    ...
    def get_ts_df(self) -> list:
        ...
        """
        ts_data = self.data["input_ts_data"]
    ...
    def check_backfilling(self, ts_input_name: str, testing: bool = False) -> Tuple[dict, list]:
        ...
        client = self.client
        data = self.data
        input_ts = data["input_ts"][ts_input_name]
        orig_extid = input_ts["orig_extid"]
    ...
    def run_backfilling(self):
        ...
        """
        input_ts = self.data["input_ts"]
        output_ts = self.data["output_ts"]
        calc_func = self.data["calculation_function"]

        for ts_in, ts_out in zip(input_ts.keys(), output_ts.keys()):
            if not input_ts[ts_in]["exists"]:
                self.data["input_ts_data"][ts_in] = float(ts_in)
                continue
        ...
        transform_timeseries = TransformTimeseries(self.data, ts_df)
